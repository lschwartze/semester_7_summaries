function vis = boundingBoxes(corners,s1,s2,inside_box,covered_by_box,vis,elem,coord,n,beam_source) 
    ...
    if sum(inside_box) <= 128
        out = doesIntersect(elem(inside_box,:), s2(covered_by_box,:), beam_source, s1(inside_box,:), coord,n(inside_box,:));
        %disp(length(vis)-sum(vis))
        ...
        vis=boundingBoxes(corners_smaller,s1,s2,inside_next_box,covered_by_box,vis,elem,coord,n,beam_source);
        ...
        vis=boundingBoxes(corners_bigger,s1,s2,inside_next_box,covered_by_box,vis,elem,coord,n,beam_source); 
    end
end