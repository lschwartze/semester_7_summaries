classdef spline_lens < general_lens
    ...
    methods
        function obj = spline_lens(Points,kind,param)
            sortMatrix = sortrows(Points',1)';
            obj = obj@general_lens(sortMatrix(:,1),sortMatrix(:,end));
        ...
        function value = horner(obj,a,x) % evaluate polynom
            % p(x) = a(1) + ( x * ( a(2) + x * ( a(3) ....
            %
            value = a(end);
        ...
        function sfx = evalSpline(obj,x)
            ...
            for k = 1:length(t)-1       % evaluate each subinterval
                ...
                ind = find(x>=t(k) & x<=t(k+1));    % find all x-values in 
                                                    % current interval
                sfx(ind) = obj.horner(p, x(ind)-t(k));     % evaluate for these x-values
            end   
        end
        ...
        function [p,normal,t] =  intersect(obj,light)
            ...
            dfp = obj.horner(dcoeff(end:-1:1),real(nst(idx))-allPoints(1,logical([idx02 0])));
            %Normale
            if dfp ~= 0
                normal = [1;-1/dfp];
