classdef weird_lens < general_lens
    ...
    methods
       function obj = weird_lens(fun,a,b,typ,eta_1,eta_2)
           if nargin == 4
                eta_1 = 1.00027717;
                eta_2 = 1.5168;            
            end
            obj = obj@general_lens(a,b,typ,eta_1,eta_2);
            obj.fun = fun;
        ...
        function [p,normal,t] =  intersect(obj,light)
             ...
             for int = intersec'
                ...
                if norm(p_temp(1)-light_transform.source(1)) < 1e-5 || tmp <=0 %toleranz
                    p = NaN;
                    normal = NaN;
                    t = NaN;
                    if int == intersec(end)
